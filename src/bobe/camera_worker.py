...
class CameraWorker:
    ...
    def __init__(self, reachy_mini: ReachyMini, head_tracker: Any = None) -> None:
        ...
        self._tracker_expects_rgb = self._tracker_wants_rgb(head_tracker)

        # Thread-safe frame storage
        self.latest_frame: NDArray[np.uint8] | None = None
        self.latest_frame_time: float | None = None  # time.monotonic() of capture
        self.frame_max_age_s = 5.0  # frames older than this are treated as unavailable
    ...
    @staticmethod
    def _tracker_wants_rgb(head_tracker: Any) -> bool:
        """Whether the tracker expects RGB frames instead of OpenCV-native BGR.

        Trackers may declare an ``expects_rgb`` attribute; otherwise infer it from
        the implementation: the reachy_mini_toolbox tracker wraps MediaPipe
        FaceMesh, which is documented to require RGB input, while the YOLO
        tracker consumes BGR directly.
        """
        if head_tracker is None:
            return False
        declared = getattr(head_tracker, "expects_rgb", None)
        if declared is not None:
            return bool(declared)
        module = type(head_tracker).__module__.lower()
        return "mediapipe" in module or "reachy_mini_toolbox" in module

    def get_latest_frame(self) -> NDArray[np.uint8] | None:
        """Get the latest fresh frame (thread-safe).

        Returns None when no frame has been captured yet or when the newest
        frame is older than ``frame_max_age_s`` (camera stalled/died) so stale
        frames are never served as current.
        """
        with self.frame_lock:
            if self.latest_frame is None or self.latest_frame_time is None:
                return None
            if time.monotonic() - self.latest_frame_time > self.frame_max_age_s:
                return None
    ...
    def working_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Get frame from robot. Capture errors are treated like a missing
                # frame so the return-to-neutral logic below still runs.
                try:
                ...
                if frame is not None and self.is_head_tracking_enabled and self.head_tracker is not None:
                    tracker_frame = frame
                    if self._tracker_expects_rgb:
                        # MediaPipe FaceMesh expects RGB; get_frame() delivers BGR
                        tracker_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    eye_center, _ = self.head_tracker.get_head_position(tracker_frame)
                ...
                # Handle smooth interpolation (works for face-lost, tracking-disabled
                # and camera-dead cases). This must run even when get_frame() returns
                # None or raises, so offsets still return to neutral after the camera
                # stream dies.
