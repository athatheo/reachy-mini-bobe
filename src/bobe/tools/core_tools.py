...
from bobe.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from bobe.config import config  # noqa: F401
from bobe.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from bobe.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "bobe.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"bobe.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
