...
def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use local vision model instead of gpt-realtime-2.1 vision",
    )
...
def handle_vision_stuff(args: argparse.Namespace, current_robot: ReachyMini) -> Tuple[CameraWorker | None, Any, Any]:
    ...
    By default, vision is handled by gpt-realtime-2.1 when camera tool is used.
    ...
    if not args.no_camera:
        ...
        if args.head_tracker is not None:
            if args.head_tracker == "yolo":
                from bobe.vision.yolo_head_tracker import HeadTracker
        ...
        if args.local_vision:
            try:
                ...
                vision_manager = initialize_local_vision()
            ...
            logging.getLogger(__name__).info(
                "Using gpt-realtime-2.1 for vision (default). Use --local-vision for local processing.",
            )
